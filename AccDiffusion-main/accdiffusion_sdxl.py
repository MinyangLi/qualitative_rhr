...
# print(f'the os.path.dirname is {os.path.dirname(__file__)}')
# print(f'the __file__ is {__file__}')
# print(f'the sys.path is {sys.path}')
...
class AccDiffusionSDXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    ...
    ###################################################### Phase Initialization ########################################################
        ...
        image_lr_save_path = f'{result_path}/{image[0].size[0]}_{image[0].size[1]}.jpg'
    ...
    #########################################################################################################################################
                ...
                if not output_type == "latent":
                    ...
                    image = self.image_processor.postprocess(image, output_type=output_type)
                    print(f'the resolution of generated image is {image[0].size}')
                    image[0].save(f'{result_path}/AccDiffusion.jpg')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_ckpt',default='diffusion_models/stable-diffusion-xl-base-1.0')
    parser.add_argument('--seed', type=int, default=2024)
    parser.add_argument('--prompt', default="A stunningly beautiful woman with perfect, delicate features, golden hair cascading in soft waves, warm amber eyes that glow with kindness, wearing a luxurious, intricately embroidered dress with pearl accents, standing in a grand marble hall bathed in soft sunlight, highly detailed, photorealistic, serene and majestic")
    ...
    # parser.add_argument('--resolution', default='2048,2048', help="target resolution")
    parser.add_argument('--use_multidiffusion', default=True, action='store_true', help="use multidiffusion or not")
    parser.add_argument('--use_guassian', default=True, action='store_true', help="use guassian or not")
    parser.add_argument('--use_dilated_sampling', default=True, action='store_true', help="use dilated sampling or not")
    parser.add_argument('--use_progressive_upscaling', default=True, action='store_true', help="use progressive upscaling or not")
    parser.add_argument('--shuffle', default=True, action='store_true', help="shuffle or not")
    parser.add_argument('--use_skip_residual', default=True, action='store_true', help="use skip_residual or not")
    ...
    parser.add_argument('--use_md_prompt', default=True, action='store_true', help="use md prompt or not")
    ...
    print(f'this is AccDiffusion working')
    pipe = AccDiffusionSDXLPipeline.from_pretrained(args.model_ckpt, variant="fp16", torch_dtype=torch.float16).to(f"cuda:{args.cuda_index}")
    generator = torch.Generator(device=f"cuda:{args.cuda_index}")
    ...
    with open(args.prompt, "r") as f:
        ...
        for pair in prompt_pairs:
            ...
            for resolution in [resolutions[1]]:
                ...
                images = pipe(pair["prompt"], negative_prompt=pair["negative_prompt"], generator=generator,
                    ...
                    shuffle=args.shuffle, result_path=output_path, 
