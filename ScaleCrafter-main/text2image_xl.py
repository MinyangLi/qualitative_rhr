...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="diffusion_models/stable-diffusion-xl-base-1.0",
    ...
    parser.add_argument("--config", type=str, default="./ScaleCrafter-main/configs/sdxl_4096x4096.yaml")
    parser.add_argument(
        "--logging_dir",
        type=str,
        default=output_dir,
    ...
    parser.add_argument("--disable_freeu", action="store_true", help="disable freeU", default=True)
    ...
    for i in range(num_batches):
        ...
        for n in range(config.num_iters_per_prompt):
            ...
            #     image.save(img_path)
                ...
                #     f.writelines([prompt, ])
            for idx, (image, prompt) in enumerate(zip(images, output_prompts)):
                # total_num = total_num + 1
                filename=filenames[idx+i*inference_batch_size]
                output_path = os.path.join(output_dir,filename)
                saved_dir = os.path.join(output_path,f'{config.pixel_height}_{config.pixel_width}_jpg')
                img_path = os.path.join(saved_dir, "ScaleCrafter.jpg")
                image.save(img_path)
